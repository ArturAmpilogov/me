...
export const Education = ({primary, secondary, title, ...props}: EducationProps): JSX.Element => {
    ...
    const educationRecordToListItem = (rec: EducationRecord) => {
        return <li className={styles.li} key={`${rec.name}-${rec.dateOfIssue}`}>
            { 
                rec.link 
                ? <a href={rec.link} target="_blank" rel="noreferrer" className={styles.name}>{rec.name}</a> 
    ...
    const secondryEducation = secondary && <ul className={styles.ul}>{secondary.map(educationRecordToListItem)}</ul>;
